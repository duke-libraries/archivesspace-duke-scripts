...
#Script reads CSV input with following columns: resource record URI, note persistent ID, eadid, finding_aid_status, title, dates, new accessrestrict text, local_access_restriction_type, restriction_end_date
#Script will locate notes by note_PID (column 2) and replace existing note text with text from CSV (column 6) and supply local access restriction type and end date if supplied in CSV (columns 7-8)
#Script will write out log of actions to CSV file including: URI, note PID, eadid, finding_aid_status, updated text, local access restriction type, restriction end date, success/failure
...
def replace_note_by_id():
    ...
    #write a header row
    writer.writerow(["URI", "note_persistent_id", "eadid", "finding_aid_status", "note_text", "local_access_restriction_type","restriction_end_date","update_status"])
    ...
    for row in csvfile:
        ...
        note_text = row[6]
        local_access_restriction_type = row[7]
        restriction_end_date = row[8]
        ...
        for note in resource_json['notes']:
            if note['jsonmodel_type'] == 'note_multipart':
                if note['persistent_id'] == persistent_id:
                    ...
                    note['rights_restriction'] = {"end": restriction_end_date,"local_access_restriction_type": [local_access_restriction_type]}
        ...
        row.append(local_access_restriction_type)
        row.append(restriction_end_date)
        try:
